...
const TestManagementPage: React.FC = () => {
    ...
    const changePremium = async (change: boolean, changId: number) => {
        try {
            const res = await http.post(`exams/${changId}/change-premium`,
                {}, {params: {premium: change}})
            if (res.status === 200) {
                fetchData()
            }
        } catch (error) {
  ...
  const handleDeleteExam = async (testId: number | "") => {
    ...
    return (
        <div className="min-h-screen flex bg-gray-100">
            <LeftSidebarAdmin customHeight="h-auto w-45"/>
                            ...
                            return (
                                <React.Fragment key={test.id}>
                                    ...
                                    >
                                        ...
                                        <td onClick={() => toggleRowExpand(test.id!)}
                                        ...
                                        <td className="py-3 px-4 text-center">
                                            <input
                                                type="checkbox"
                                                checked={test.isPremium}
                                                onChange={() =>{
                                                    changePremium(!test.isPremium, test.id)
                                                }}
                                                style={{width: "20px", height: "20px", accentColor: "#4285F4"}}
                                            />
                                        </td>
                                    </tr>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
                    <div className="bg-white p-6 rounded-lg shadow-lg w-96 relative">
                        ...
                        <button
                            className="flex items-center gap-2 border border-gray-400 px-4 py-2 rounded bg-gray-200 hover:bg-gray-300 transition">
                            <FaUpload/>
                            Upload File
                        </button>
