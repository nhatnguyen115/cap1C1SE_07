...
public interface UserTestAttemptRepository extends JpaRepository<UserTestAttempt, Integer> {
    ...
    @Query(value = """
                WITH ranked_attempts AS (
                                 SELECT
                                     ...
                                     CAST(ROUND(EXTRACT(EPOCH FROM (uta.end_time - uta.start_time)) / 60) AS BIGINT) as totalTime,
                                     ROW_NUMBER() OVER (PARTITION BY uld.user_id ORDER BY uta.total_score DESC) as rank
    ...
    List<UserTestAttempt> findByUserIdAndCompleteTrue(UUID userId);

    Integer countByUserIdAndExamIdAndCompleteTrue(UUID userId, Integer examId);
